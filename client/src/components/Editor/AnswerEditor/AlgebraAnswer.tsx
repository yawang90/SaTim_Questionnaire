        ...
        <Box>
            ...
            {conditions.map((cond, idx) => (
                <Box key={idx} display="flex" alignItems="center" gap={1} mb={1}>
                    {idx > 0 && (
                        <Select value={cond.logic || "and"} onChange={(e) => handleChange(idx, "logic", e.target.value)} size="small" sx={{ minWidth: 60 }}>
                            {logics.map((l) => (
                    ...
                    <MathInput value={cond.value} onChange={(val) => handleChange(idx, "value", val)} placeholder="z. B. α + β = γ oder 2x + 3y = 5"/>
