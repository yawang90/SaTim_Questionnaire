...
export function normalize(rhs: string): string {
    return rhs
        .replace(/\\cdot/g, '*')
        .replace(/:/g, '/')
        .replace(/([0-9)])x/g, '$1*x')   // 5x → 5*x
        .replace(/x\(/g, 'x*(');         // x(5) → x*(5)
}

function isLinearTerm(term: string): boolean {
    const xCount = (term.match(/x/g) || []).length;
    if (xCount > 1) return false;
    return !(term.includes('*x*') || term.includes('x*(') || term.includes(')*x'));

}

export function extractLinearMC(answer: Answer): { m: string; c: string } | null {

    if (typeof answer.value !== "string") return null;

    const sanitized = answer.value.replace(/\s+/g, "");
    if (!sanitized.startsWith("y=")) return null;

    const rhs = normalize(sanitized.slice(2));
    ...
    for (const term of terms) {
        if (!isLinearTerm(term)) return null;
    ...
    return {
        m: simplifySum(mTerms) || "0",
        c: simplifySum(cTerms) || "0",
    };
}

function simplifySum(terms: string[]): string {
    if (terms.length === 0) return "0";

    const cleaned = terms.filter(t => t !== "0" && t !== "+0" && t !== "-0");
